...
public interface HorarioRepository extends JpaRepository<Horario, Long>{
	...
	@Query("SELECT h FROM Horario h "
			...
			+ "order by h.datahora")
	List<Horario> listarPorDia(int ano,int mes,int dia, Empresa empresa);
	...
	@Query("SELECT h FROM Horario h "
			...
			+ "and DATE_PART('day', h.datahora) = :dia "
			+ "and h.empresa = :empresa "
			+ "and h.cliente = :cliente "
			+ "order by h.datahora")
	List<Horario> buscarHorariosPorClienteEmpresa(int ano,int mes,int dia, Empresa empresa, Cliente cliente);
